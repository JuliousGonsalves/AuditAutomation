...
def generate_pdf(per_standard_stats, overall_score, total_passed, total_controls, file_name):
    """Generate PDF with description, big colored score, and table"""
    ...
    # Add extra spacer to prevent overlap with controls
    elements.append(Spacer(1, 20))

    # Passed/total controls
    ...
    for standard, stats in per_standard_stats.items():
        display_name = standard_display_names.get(standard, standard)
        table_data.append([display_name, stats['passed'], stats['failed'], stats['score']])

    # Sort table data for consistent order
...
# Main
# ------------------------------
def fetch_security_scores(region_name=None, accountId=None):
    """Fetch Security Hub data"""
    client_params = {}
    if region_name:
        client_params['region_name'] = region_name
    client = boto3.client('securityhub', **client_params)
    ...
    for standard, controls in standardsDict.items():
        ...
        per_standard_stats[standard] = {'passed': passed, 'failed': failed, 'score': score}
...
if __name__ == "__main__":
    ...
    per_standard_stats, overall_score, total_passed, total_controls = fetch_security_scores(REGION_NAME, ACCOUNT_ID)
